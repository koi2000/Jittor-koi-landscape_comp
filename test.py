...
parser.add_argument("--epoch", type=int, default=58, help="epoch to start training from")
...
# )
val_dataloader = ImageDataset(opt.input_path, mode="", transforms=transforms).set_attrs(
    batch_size=10,
    shuffle=False,
    num_workers=1,
)
...
def eval(epoch, writer):
    ...
    for i, (_, real_A, photo_id) in enumerate(val_dataloader):
        ...
        for idx in range(fake_B.shape[0]):
            ig1 = fake_B[idx]
            # ig1 = ig1.reshape(1,3,384,512)
            ig2 = ig1.transpose(1, 2, 0)[:, :, ::-1]
            # for idxx in range(ig1.shape[0]):
            # save_image(ig1, f"{opt.output_path}/images/test_fake_imgs/epoch_{epoch}/{photo_id[idx]}.jpg", nrow=1)

            strlit = photo_id[idx].split("\\")
            if(len(strlit)==1):
                strlit = photo_id[idx].split("/")
           
            path = opt.output_path+"/" + strlit[-1] + ".jpg"
            # print(path)
            # if not os.path.exists(path):
            #     os.makedirs(path)
            # cv2.imwrite(f"./val/image/{photo_id[idx]}.jpg",ig2)
            cv2.imwrite(path, ig2)
